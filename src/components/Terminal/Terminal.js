...
class Terminal extends React.Component{
    ...
    getChildren(){
        if(this.children){
            return this.children.length - this.children.filter(child => child.type === TerminalSkip).length;
        } else {
            return 0
        }
    }
    ...
    handleSkip(){
        ...
        if(childNumber < this.numberOfItems){
            if(this.children[childNumber] && this.children[childNumber].type === TerminalItem){
    ...
    handleGlobals(child){
        this._isMounted && this.setState({childNumber: this.state.childNumber + 1});
    ...
    handleTyping(){
        const childNumber = this.state.childNumber;
        if(childNumber < this.numberOfItems && this.children[childNumber].type !== TerminalItem){
            this.setState({childNumber: this.state.childNumber + 1});
            this.handleTyping();
        ...
        return(
            ...
            if(child.type === TerminalSkip && this._isMounted)
                    return <TerminalSkip
                        ...
                        ref={(ref) => {this[`child-${this.children.indexOf(child)}`] = ref;}}
