...
class Terminal extends React.Component {
  ...
  constructor(props) {
    ...
    this.numberOfItems = this.props.children.filter(
      (child) => child.type.name === "TerminalItem"
    );
  ...
  render() {
    ...
    return (
      <>
        {children
          .filter((child) => child.type.name === "TerminalItem")
          .map((child, index) => (
            <TerminalItem
        ...
        {children
          .filter((child) => child.type.name === "TerminalSkip")
          .map((child, index) => (
            <TerminalSkip
              key={`child-${children.indexOf(child).toString()}`}
              ref={(ref) => {
                this[`child-${children.indexOf(child)}`] = ref;
              }}
