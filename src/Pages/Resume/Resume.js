...
const Resume = (props) => {
      ...
      let isMounted = true;
        const getData = async () => {
            Promise.all([
                ...
                setHeaderImage([
                    res[0].data.find(function(e) { return e.name === 'ResumePageImage'; }).image.full_size,
                    res[0].data.find(function(e) { return e.name === 'ResumePageImageMobile'; }).image.full_size
                ])
                ...
                setresume(res[3].data.find(function(e) { return e.name === 'Resume'; }));

                setTechExperience([
                    res[4].data.find(function(e) { return e.name === 'Proficient'; }),
                    res[4].data.find(function(e) { return e.name === 'Familiar'; }),
                    res[4].data.find(function(e) { return e.name === 'Exposure'; }),
                    res[4].data.find(function(e) { return e.name === 'Technical'; }),
                ])
    ...
    return(
        ...
        <TopMenu
          ...
          burgerColor={"black"}
        />
        <div className="TopPicture" style={{
        ...
        <div className="outerContent">
          <div className="innerContent">
            ...
            {EducationReturned && Education.map((item, index) => {
                return <div className="EducationSection" key={index}>
              <a
                target="_blank"
            ...
            {WorkExperienceReturned && WorkExperience.map((item, index) => {
                return <div key={index} className="WorkSection">
                <a
                  target="_blank"
                  rel="noopener noreferrer"
                  href={item.image.link}
                >
                  <img className="LargeIcon" src={item.image.image.full_size} alt={item.image.description} />
                </a>
                <b>{item.title}</b>
                <span className="alignRight">{item.dates}</span>
                <br />
                ...
                <p>
                  {item.description}
                    ...
                    <button className="PaperButton" onClick={() => window.open(item.optionalFile.fileupload)}>
                    {item.fileText}
                  </button>
                  </div>
                  : <></>
            ...
            <div className="TechnicalSection">
              ...
              <br />
              {techExperienceReturned && techExperience[0].skills.map((skill, index) => {
                  return <div key={index}>
                    <span className="indent">{skill.name}</span>
                    <span className="alignRight">{skill.experience} Years of Experience</span>
                    <br />
                    <br />
                    </div>
              })}
              ...
              <br />
              {techExperienceReturned && techExperience[1].skills.map((skill, index) => {
                  return <div key={index}>
                    <span className="indent">{skill.name}</span>
                    <span className="alignRight">{skill.experience} Years of Experience</span>
                    <br />
              ...
              <div className="alignCenter" style={{
                ...
                {techExperienceReturned && techExperience[2].skills.map((skill,index) => {
                    return <span key={index} className="sideBySide">{skill.name}</span>
                })}
              </div>
              <br />
              <hr className="TitleDivider" />
              ...
              <ul>
                {techExperienceReturned && techExperience[3].skills.map((skill, index) => {
            ...
            <div className="alignCenter" style={{ paddingBottom: "30px" }}>
              <button className="FullResumeButton" onClick={() => window.open(resume.fileupload)}>
                Full Resume
              </button>
            </div>
          </div>
